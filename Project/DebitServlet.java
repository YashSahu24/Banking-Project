  ...
  try
   {
     ...
     if(rs.next())
     {
       ...
       String message="Dear Customer, Your A/c No. "+acnumber+" is Debited for Rs. "+amount+" on "+date+"(Your transaction ID:"+tn+" ) Account New Balance = "+amount1;
       ...
       out.println("<br><br>Debited Amount : "+amount);
       out.println("<br><br>Account Balance : "+amount1+"</h3></center></div></body></html>"); 
       }
       else
       {
        out.println("<center><p style='color:red;'><br>InSufficient Balance IN Account....</p></center>");
        RequestDispatcher rd=req.getRequestDispatcher("adminhome.html");
        rd.include(req,res);
        out.println("<html><head><style>h3{color:red; font-size: 1.7em}</style></head><body><div class='box2'");
       out.println("<center><h3><br>Account Balance is Less than Debit Amount"); 
       out.println("</h3></center></div></body></html>");
       }
     }
     else
     { 
